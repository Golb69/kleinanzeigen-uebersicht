...
# Wird nur noch als EINMALIGER Startwert für "default_blacklist" in
# links.json verwendet (bei der Migration von links.txt). Danach ist
# links.json die Quelle der Wahrheit – Änderungen hier haben dann keine
# Wirkung mehr. Bearbeite die Standard-Blacklist stattdessen im Editor.
EXCLUDE_KEYWORDS = [
...
def contains_excluded_words(text: str, blacklist: list[str]) -> bool:
...
def load_topics_config(json_path: Path, txt_path: Path) -> dict:
    """
    Lädt die Themen-/Link-Konfiguration aus links.json (strukturiertes
    Format mit einer eigenen Blacklist pro Link, das der mobile Editor
    schreibt/liest).

    Existiert links.json noch nicht, wird sie EINMALIG aus der alten
    links.txt migriert: jeder Link bekommt eine leere eigene Blacklist,
    die bisherigen EXCLUDE_KEYWORDS werden als "default_blacklist"
    (gilt für alle Links) übernommen. Ab dann ist links.json die
    alleinige Quelle der Wahrheit.
    """
    if json_path.exists():
        data = json.loads(json_path.read_text(encoding="utf-8"))
        data.setdefault("default_blacklist", list(EXCLUDE_KEYWORDS))
        data.setdefault("topics", [])
        for topic in data["topics"]:
            topic.setdefault("links", [])
            for link in topic["links"]:
                link.setdefault("blacklist", [])
        return data

    print(f"[INFO] '{json_path.name}' nicht gefunden – migriere einmalig aus '{txt_path.name}'.")
    legacy_topics = load_links(txt_path) if txt_path.exists() else {}
    data = {
        "default_blacklist": list(EXCLUDE_KEYWORDS),
        "topics": [
            {
                "name": name,
                "links": [{"url": url, "blacklist": []} for url in urls],
            }
            for name, urls in legacy_topics.items()
        ],
    }
    json_path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[OK] '{json_path.name}' erstellt. Ab jetzt bitte den Editor bzw. "
          f"'{json_path.name}' zum Bearbeiten verwenden.")
    return data
...
def parse_listing_page(
    html: str, session: requests.Session, blacklist: list[str]
    ...
    for a in soup.find_all("a", href=True):
        ...
        if contains_excluded_words(text, blacklist):
            continue
...
def fetch_all_pages(
    session: requests.Session, base_url: str, blacklist: list[str]
) -> list[dict]:
    all_ads: list[dict] = []
    ...
    ads, _ = parse_listing_page(resp.text, session, blacklist)
    for ad in ads:
...
def build_rss_feeds(cache: dict, feeds_dir: Path, site_base_url: str) -> None:
    ...
    for topic, ads in cache.get("ads", {}).items():
        ...
        for ad in ad_list[:MAX_ADS_PER_TOPIC]:
            ...
            # Kompletter Beschreibungsinhalt wird verlinkt, damit ein Klick
            # auch INNERHALB der Inoreader-Vorschau (nicht nur auf den Titel)
            # direkt zur Kleinanzeigen-Seite führt.
            description_html = f"<a href='{link}'>{image_html}{description}</a>"
...
def main() -> None:
    config = load_topics_config(LINKS_JSON_FILE, LINKS_FILE)
    default_blacklist = config.get("default_blacklist", [])
    topics = config.get("topics", [])
    cache = load_cache(CACHE_FILE)

    total_links = sum(len(t.get("links", [])) for t in topics)
    ...
    for topic in topics:
        ...
        for link in topic.get("links", []):
            url = link["url"]
            # Eigene Blacklist des Links + globale Standard-Blacklist,
            # Duplikate raus, Reihenfolge bleibt stabil.
            blacklist = list(dict.fromkeys(default_blacklist + link.get("blacklist", [])))

            if not needs_refetch(cache, url):
                continue
            try:
                print(f"[INFO] Hole {topic_name}: {url}")
                ads = fetch_all_pages(session, url, blacklist)
                print(f"[OK] {topic_name}: {url} -> {len(ads)} gefilterte Anzeigen gesamt")
                update_cache_with_ads(cache, topic_name, ads)
